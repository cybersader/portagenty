...
6. A root outside the service cgroup cannot reconcile as owned. Descendants outside it produce split containment; a descendant deliberately launched by `build-contained` beneath `agent-work.slice` is reported as an external bounded scope rather than misreported as service-owned.
