...
An **experimental Linux-only resource-supervision MVP** is also implemented: explicit `--supervise` launches use transient systemd user services and cgroup v2, private tmux/Zellij targets, machine-local ownership receipts, soft guardrails (`MemoryHigh`, CPU quota, `TasksMax`), CLI/TUI metrics, event warnings, graceful non-force stop, and separately confirmed whole-cgroup force-kill. Unsupported platforms report that capability honestly. No Portagenty daemon, listener, history collector, or unattended telemetry service was added.
...
- **Supervision is explicit and capability-aware.** Normal Enter/launch remains unsupervised. Linux supervision may use transient systemd user services, but Portagenty must not become a resident daemon, listener, log monitor, restart manager, or history service. Existing live multiplexer sessions are never retroactively claimed.
