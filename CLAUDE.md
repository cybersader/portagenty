...
An **experimental Linux-only resource-supervision MVP** is also implemented: eligible idle UUID-backed TUI rows use supervision-first Enter, while CLI supervision remains explicit. Only `kind = "claude-code"` selects Claude containment (`claude-code.slice`) and the standard `3G` MemoryHigh / `5G` MemoryMax / `512MiB` MemorySwapMax / `800%` CPU / `1200` tasks policy; names and commands never imply it. Transient systemd user services, v2 workload-anchor receipts, exact root proof, and bounded descendant cgroup checks provide private tmux/Zellij targets, metrics, event warnings, graceful non-force stop, and separately confirmed whole-cgroup force-kill. Existing v1 services are legacy/restart-required attach-only; escaped descendants produce split containment and withhold whole-workload control. Only proven-safe non-creating capability failure may fall back loudly to an ordinary TUI launch; ownership ambiguity and post-creation failures remain fail-closed. Unsupported platforms report capability honestly. No Portagenty daemon, listener, history collector, or unattended telemetry service was added.
...
- **Supervision is proof-based and capability-aware.** Eligible idle UUID-backed rows use supervision-first TUI Enter; ordinary `pa launch` remains unsupervised unless CLI flags explicitly request supervision. Live ordinary sessions are attached as-is and never retroactively claimed. Automatic fallback is allowed only after non-creating preflight proves both that ownership is unambiguous and that supervision capability/runtime is unavailable; after creation may have begun, fail closed. Linux supervision may use transient systemd user services, but Portagenty must not become a resident daemon, listener, log monitor, restart manager, or history service.
