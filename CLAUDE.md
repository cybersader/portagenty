...
An **experimental Linux-only resource-supervision MVP** is also implemented: eligible idle UUID-backed TUI rows use supervision-first Enter with recommended soft guardrails, while CLI supervision remains explicit. Transient systemd user services and cgroup v2 provide private tmux/Zellij targets, machine-local ownership receipts, metrics, event warnings, graceful non-force stop, and separately confirmed whole-cgroup force-kill. Only proven-safe non-creating capability failure may fall back loudly to an ordinary TUI launch; ownership ambiguity and post-creation failures remain fail-closed. Unsupported platforms report capability honestly. No Portagenty daemon, listener, history collector, or unattended telemetry service was added.
