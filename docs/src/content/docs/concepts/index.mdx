...
session ID, so Enter can create a private supervised target with `12G` Memory High,
`300%` CPU, and `1200` tasks. Missing IDs keep ordinary Enter and can be upgraded
explicitly with `S`; malformed IDs require manual correction. Owned rows attach to
the exact receipt target, live ordinary rows attach without relaunch or ownership,
and an idle stale receipt can be exactly replaced only after signal-free proof that
its old invocation and target are absent. If non-creating capability preflight fails
with ownership already known safe, routine Enter announces an ordinary unsupervised
fallback; later or ambiguous failures remain fail-closed.
