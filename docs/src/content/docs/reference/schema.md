...
continue to work and retain ordinary Enter. Supervision-first Enter applies only
to idle rows with a valid ID. `S` can explicitly confirm-add an ID to a writable
legacy workspace while preserving comments and session blocks; an existing
malformed ID never receives supervised Enter and is never silently replaced. The
ID is designed to be committed alongside the file so external tools (like
`claudecode-project-sync`) can track a workspace across folder moves and
