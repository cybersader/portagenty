...
continue to work. When `S` is used for supervision, the TUI can explicitly
confirm-add an ID to a writable legacy workspace while preserving comments and
session blocks; an existing malformed ID is never silently replaced. The ID is
designed to be committed alongside the file so external tools (like
